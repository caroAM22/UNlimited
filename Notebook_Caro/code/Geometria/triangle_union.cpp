...
vector<ptT> interSegSeg(LineT l1, LineT l2){
    if(eq(l1.vec().cross(l2.vec()), 0)){
        ...
        if(l == r)
            return {l};
        else return lexComp(l, r) ? vector<ptT>{l, r} : vector<ptT>();
    }
    else{
...
dbl union_area(vector<tuple<ptT, ptT, ptT> > triangles){
    ...
    for(size_t i = 0; i < triangles.size(); i++){
        ...
        segments[3 * i] = lexComp(a, b) ? LineT(a, b) : LineT(b, a);
        segtype[3 * i] = get_segtype(segments[3 * i], c);
        segments[3 * i + 1] = lexComp(b, c) ? LineT(b, c) : LineT(c, b);
        segtype[3 * i + 1] = get_segtype(segments[3 * i + 1], a);
        segments[3 * i + 2] = lexComp(c, a) ? LineT(c, a) : LineT(a, c);
